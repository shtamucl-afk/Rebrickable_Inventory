...
# --- CONFIG ---
API_KEY = 'a0aa38b8fd6c37848d60cfb20dfc4463'  # Replace with your actual API key
...
# --- Load colors and build color_name -> rgb map ---
colors_df = pd.read_csv("colors.csv")
color_rgb_map = dict(zip(colors_df['name'], colors_df['rgb']))
...
with st.sidebar:
    ...
    manual_set_id = st.text_input("Or enter a Set ID manually", value="")
    ...
    st.text_input("Search Part ID", key="part_id_search")

    # Prepare filter lists
    # These will be updated after df is loaded, but are needed for UI
    cs = st.session_state.color_search.strip().lower()
    cats = st.session_state.category_search.strip().lower()
    # Placeholders for all_colors and all_categories (will be updated after df is loaded)
    filtered_colors = []
    filtered_categories = []

    # Multiselects will be updated after df is loaded
...
if set_info and not df.empty:
    ...
    with st.sidebar:
        ...
        st.markdown("**Color (Tick to Select):**")
        cols = st.columns(6)
        selected_colors = st.session_state.get("color_filter", [])

        for idx, color_name in enumerate(filtered_colors):
            rgb = color_rgb_map.get(color_name, "FFFFFF")
            hex_rgb = f"#{rgb}"
            is_selected = color_name in selected_colors
            btn_label = "✓" if is_selected else ""
            with cols[idx % 6]:
                if st.button(btn_label, key=f"colorbtn_{color_name}", help=color_name):
                    if is_selected:
                    ...
                    st.rerun()
                # Show color swatch below the button (no label)
                st.markdown(
                    f"""
                    <div style='width:32px; height:32px; border-radius:8px; background:{hex_rgb}; border:2px solid #ccc; margin:auto; margin-bottom:4px'></div>
                    """,
                    unsafe_allow_html=True
                )
    ...
    for _, row in df.iterrows():
        ...
        if st.session_state.part_search and st.session_state.part_search.lower() not in row['part_name'].lower():
            continue
        if st.session_state.part_id_search and st.session_state.part_id_search.lower() not in row['part_num'].lower():
            continue
